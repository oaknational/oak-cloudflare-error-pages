...
- Edit `src/template.html` (the shared layout) or `src/pages.json` (each page's title, heading, body and Cloudflare token), then run `npm run build` to regenerate `docs/`. Commit both — `docs/` is what GitHub Pages serves, and CI fails if it doesn't match `src/`. Don't hand-edit `docs/`.
...
Every file in `docs/` is generated by `npm run build` from `src/template.html` plus one entry in `src/pages.json` (`file`, `title`, `heading`, `body`, the Cloudflare `types` it's wired to and the `tokens` it must carry). To add a page, add an entry and run the build; to change the layout, edit the template and rebuild — all pages update together. Pages must stay under 1.5 MB, contain `<head>…</head>`, and must **not** include a `referrer` meta tag (it breaks challenges).
...
`check:pages` takes the list of pages, types and tokens from `src/pages.json`, so a page that exists in `docs/` but not in `src/pages.json` (or vice versa) fails the check.
